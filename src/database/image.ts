...
export const getImageFromCollection = async (imageId: string): Promise<ImageInfo | null> => {
    try {
        const db: mongodb.Db = getMongoClient().db(MONGODB_DB);
        const collection: mongodb.Collection<ImageInfo> = db.collection("image_data");
...
export const updateUserAccessToken = async (imageId: string, imageUrl:string , createdAt?: Date): Promise<void> => {
    try {
        const db: mongodb.Db = getMongoClient().db(MONGODB_DB);
        const collection: mongodb.Collection<ImageInfo> = db.collection("image_data");
        ...
        await collection.updateOne(
            { image_id: imageId },
            { $set: { image_url: imageUrl, created_at: createdAt } },
        );
    } catch (error) {
